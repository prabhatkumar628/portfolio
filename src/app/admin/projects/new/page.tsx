...
import { useCreateAdminProject } from "../../../../hooks/useAdminProjects";
...
export default function CreateProjectPage() {
  ...
  const handleRemoveThumbnail = () => {
    ...
    form.setValue("thumbnail", { url: "", public_id: "" });
  };

  // ─── Submit Form ─────────────────────────────────────────
  const { mutate: createProject, isPending } = useCreateAdminProject();
  const onSubmit = async (values: ProjectFormInputs) => {
    ...
    try {
      ...
      values.thumbnail = thumbnailData;
                  ...
                  {thumbnailPreview && (
                    <div className="relative w-full max-w-md aspect-video rounded-2xl bg-white/5 border border-white/10 overflow-hidden">
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 w-8 h-8 rounded-full bg-red-500/80 hover:bg-red-500 text-white flex items-center justify-center transition-all z-10"
                      >
                        <X size={16} />
                      </button>
